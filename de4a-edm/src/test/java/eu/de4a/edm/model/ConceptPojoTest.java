...
import eu.de4a.edm.error.EDE4ADataElementResponseErrorCode;
...
public final class ConceptPojoTest
{
  ...
  public void testPilotGBM ()
  {
    final ConceptPojo x = ConceptPojo.builder ()
                                     .id ("ConceptID-1")
                                     .name (EDE4AConcept.NAMESPACE_URI, "CompanyData")
                                     .addChild (y -> y.id ("ConceptID-2").name (EDE4AConcept.COMPANY_NAME))
                                     .addChild (y -> y.id ("ConceptID-3").name (EDE4AConcept.COMPANY_TYPE))
                                     .build ();
    _testWriteAndRead (x);
  }
  ...
  public void testValues ()
  {
    ...
    final ConceptPojo x = ConceptPojo.builder ()
                                     ...
                                     .addChild (_concept ().valueErrorCode (EDE4AErrorCode.DD_003))
                                     .build ();
    _testWriteAndRead (x);
  }
  ...
  public void testBuilderFromInstanceNested ()
  {
    ...
    final ConceptPojo y = x.cloneAndModify (aHdl -> {
      if (EDE4AConcept.COMPANY_NAME.getAsQName ().equals (aHdl.name ()))
        aHdl.valueText ("My company name");
      else
        if (EDE4AConcept.COMPANY_TYPE.getAsQName ().equals (aHdl.name ()))
          aHdl.valueText ("My company type");
        else
          if (!aHdl.name ().getLocalPart ().equals ("CompanyData"))
            aHdl.valueErrorCode (EDE4ADataElementResponseErrorCode.DP_ELE_001);
    });
